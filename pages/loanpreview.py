...
with col1:
    ...
    st.write("")
    full_price = price * 1e5
    principal_in_lakhs = st.slider(
        "Principal amount (in lakhs)", min_value=1.0, max_value=price, step=0.1
    )
...
with col3:
    ...
    if payable_interest > 1e5:
        st.markdown(
            f"""##### Payable Interest: ₹{
                payable_interest / 1e5:.2f} Lakh"""
        )
    else:
        st.markdown(
            f"""  # Payable Interest: ₹{
                payable_interest / 1e3: .2f} Thousand"""
        )
st.divider()
