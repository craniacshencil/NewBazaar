...
image_urls = []
client = imgbbpy.SyncClient(IMG_BB_KEY)


def urlgen(image_urls, img_path):
    image = client.upload(
        file=img_path, expiration=2592000
    )  # Link valid for 30 days since creation
...
uploaded_images = st.file_uploader(
    "Upload images",
    type=["jpg", "jpeg", "png", "AVIF", "bmp", "webp"],
    accept_multiple_files=True,
)
st.info("Upload Landscape photos only")
display_button = st.columns(5)[2].button(
    "Display Images", use_container_width=True, on_click=set_images_displayed
)
...
if st.session_state["images_displayed"]:
    for uploaded_image in uploaded_images:
        ...
        if image.mode in (
            "RGBA",
            "P",
        ):  # Converting photos with transparent backgrounds to jpg
            print("converting image type")
        ...
        print("url for image was created")
        with col_no[i]:
...
if finish:
    if st.session_state["images_displayed"]:
        ...
        if response.status_code == 200:
            ...
            dispimgpath = "temp_storage//dispimage.jpg"
            new_image.save(dispimgpath)
            new_image = client.upload(
                file=dispimgpath, expiration=2592000
            )  # Link valid for 30 days since creation
