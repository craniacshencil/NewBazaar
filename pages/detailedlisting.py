...
with col1:
    ...
    for url in image_urls:
        item = dict(title="", text="", interval=None, img=url)
...
with col2:
    with stylable_container(
        ...
        st.markdown(
            f"""##### {car['Myear']} {car['Brand'].capitalize()} {car['Model'].capitalize()} {
                car['Variant'].capitalize()}"""
        )
        st.markdown(
            f"""###### {int(car['Kms'] / 1e3)}k kms · {car['Fueltype'].capitalize()} · {
                car['Transmission']} · {car['Ownerno']} owner · by {car['Seller']}"""
        )
...
if wishlist:
    ...
    wishlist_doc = {"User": st.session_state["name"], "id_in_post": car["_id"]}
...
with stylable_container(
    ...
    # Row 2:
    my_grid.text_input(
        label="Fuel-Supply System", value=Fuelsupplysystem, disabled=True
    )
    my_grid.text_input(label="Tyres", value=tyres.capitalize(), disabled=True)
    my_grid.text_input(
        label="Steering Type", value=steering_type.capitalize(), disabled=True
    )
