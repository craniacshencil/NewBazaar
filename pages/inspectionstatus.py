        ...
        with col1:
            ...
            st.caption(
                f"""{carfuel} · {cartransmission} · {
                    int(car['Kms'] / 1e3)}k kms · by {car['Seller']}"""
            )
            st.write("")
...
with column2:
    ...
    st.header("")
    col1, col2, col3, col4, col5 = st.columns([0.15, 0.18, 0.2, 0.2, 0.15])
    with col1:
    ...
    if carinspection == "Admin Approval Granted":
        ...
        with textcol:
            st.subheader(
                f"""Confirmed!! Your inspection is on {
                    inspection_date} at {inspection_time}"""
            )
    ...
    if carinspection == "Admin Approval Denied":
        ...
        st.info(
            "Possibly because of: Overpacked warehouses, fully booked timeslots etc."
        )
    ...
    if carinspection == "Inspection Successful":
        ...
        st.header("")
        to_listing = st.columns(3)[1].button(
            "Go to my listings", use_container_width=True, type="primary"
        )
        if to_listing:
