...
nav_bar = option_menu(
    None,
    ["Home", "Check Valuation", "Buy", "My Listings", "Wishlist", "Book Inspection"],
    icons=[
        "house-fill",
        "cash-coin",
        "car-front",
...
with col2:
    brand = st.selectbox("Enter the brand of your car: ", df["oem"].unique())
    # st.divider()
    yr = st.selectbox(
        "Enter registration year of car: ",
        sorted(df.loc[df.oem == brand]["myear"].unique(), reverse=True),
    )
    # st.divider()
    model = st.selectbox(
        "Enter the model of your car: ",
        df.loc[(df.oem == brand) & (df.myear == yr)]["model"].unique(),
    )
    # st.divider()
    variant = st.selectbox(
        "Enter the variant: ",
        df.loc[(df.model == model) & (df.myear == yr)]["variant"].unique(),
    )
    # st.divider()
    fueltype = st.selectbox(
        "Enter fuel: ",
        df2.loc[(df2.model == model) & (df2.myear == yr) & (df2.variant == variant)][
            "fuel"
        ].unique(),
    )
    ...
    owner = st.selectbox("Enter owner-number: ", df["owner_type"].unique())
    st.info("here 1 = first owner")
    kms = st.number_input("Enter kms driven: ", min_value=0, max_value=130000, step=1)
...
if confirmation:
    ...
    if st.session_state["kms"] == "set":
        ...
        values = [brand, yr, model, variant, fueltype, transmission, owner, kms]
        temp = df.loc[
            (df.oem == values[0])
            & (df.myear == values[1])
            & (df.model == values[2])
